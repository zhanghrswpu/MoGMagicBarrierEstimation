...
public class SimpleTool extends Object {
	 ...
	 *************************** 
	 */
	public static void printMatrix(float[][] paraMatrix) {
		if (paraMatrix.length == 0) {
			System.out.println("This is an empty matrix.");
			return;
		} else {
	 ...
	 *************************** 
	 */
	public static void printTripleMatrix(double[][][] paraMatrix) {
		if (paraMatrix.length == 0) {
			System.out.println("This is an empty matrix.");
			return;
		} else {
		...
		for(int k = 0; k < paraMatrix.length; k ++){
			for (int i = 0; i < paraMatrix[k].length; i++) {
				for (int j = 0; j < paraMatrix[k][i].length; j++) {
					System.out.print("" + paraMatrix[k][i][j] + "\t");
				}// Of for j
				System.out.println();
			}// Of for i
			System.out.println();
	...
	public static void printDoubleArray(double[] paraDoubleArray) {
		for (int i = 0; i < paraDoubleArray.length; i++) {
			System.out.printf(" %8.4f", paraDoubleArray[i]);
		}// Of for i
		System.out.println();
	}// Of printDoubleArray
	...
	/**
	 ************************* 
	 * Print a double array with 5 digits after the dot.
	 ************************* 
	 */
	public static void printDoubleArray5(double[] paraDoubleArray) {
		for (int i = 0; i < paraDoubleArray.length; i++) {
			System.out.printf("%5f ", paraDoubleArray[i]);
		}// Of for i
		System.out.println();
	}// Of printDoubleArray5
	 ...
	 ************************* 
	 */
	public static void printFloatArray5(float[] paraFloatArray) {
		for (int i = 0; i < paraFloatArray.length; i++) {
			System.out.printf("%5f ", paraFloatArray[i]);
		}// Of for i
		System.out.println();
	}// Of printDoubleArray5
	 ...
	 ***************************** 
	 * Let X ~ N(\mu, \sigma).
     * Compute ln f(x) = - (part1 + part2 + part3)/2.<br>
     * Here part1 = \frac{(x - \mu)^2}{\sigma^2}<br>
     * part2 = ln 2\pi<br>
     * part3 = 2\ln \sigma<br>
	 * Part1: from Step1 to Step4<br> 
	 * Part2 and Part3: Step5<br>
	 * @param paraError the error
	 * 
	 * @return the probability density of the value.
	 ***************************** 
	 */
	public static float logGaussPdf(float paraX, float paraMu,
			float paraSigmaSquare) {
		
		float tempDeviation = paraX - paraMu;
		
		float tempSigma = (float)Math.sqrt(paraSigmaSquare);

		float tempPart1 = tempDeviation * tempDeviation / paraSigmaSquare;

		float tempPart2 = (float)Math.log(2 * Math.PI);

		float tempPart3 = (float)(2 * Math.log(tempSigma));
		
		float resultLogGaussPdf = - (tempPart1 + tempPart2 + tempPart3) / 2;

		return resultLogGaussPdf;
	}// of logGaussPdf
	
	/**
	 ***************************** 
	 * Compute the ln pdf for an array of variables.
	 * @param paraError each element represents the error
	 * 
	 * @return the probability density of each variable.
	 * @see #logGaussPdf(double, double, double)
	 ***************************** 
	 */
	public static float[] logGaussPdf(float[] paraX, float paraMu,
			float paraSigmaSquare) {

		//Make the code shorter and more adaptive. It is also numTrainSize.
		int tempSize = paraX.length;
		float[] tempErrors = new float[tempSize];
		float[] resultLogGaussPdf = new float[tempSize];
		
		// Step 1. paraError subtracts paraMu. X = bsxfun(@minus,X,mu);
		for (int i = 0; i < tempSize; i++) {
			tempErrors[i] = paraX[i] - paraMu;
		} // of for i
		...
		float tempQ;
		float[] tempPart1 = new float[tempSize];
		for (int i = 0; i < tempErrors.length; i++) {
			tempQ = tempErrors[i] / tempSigma;
			tempPart1[i] = tempQ * tempQ;
		} // of for i
		...
		for (int i = 0; i < tempErrors.length; i++) {
			resultLogGaussPdf[i] = -(tempPart1[i] + tempPart2 + tempPart3) / 2;
		} // of for i
