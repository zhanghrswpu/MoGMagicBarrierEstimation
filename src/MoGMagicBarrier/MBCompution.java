...
public class MBCompution {
	...
	public double computeMGofMAE2(CondProb paraCP, double[][][] paraVisBasedCondPro, float[][] paraNoiseDistribution,
		...
		// SimpleTool.printDoubleArray(paraTrainVector);
		for (int y = 0; y < paraNoiseDistribution.length; y++) {// ÆÀ·ÖÏòÁ¿µÄ³¤¶È
			for (int k = 0; k < paraNoiseDistribution[0].length; k++) {// ÔëÉùµÄ¸öÊý
	...
	public static void main(String[] args) throws IOException {
		try {
			...
			tempData.computeDataVector();// convert rating matrix to rating vector
			tempData.recomputeDataset();// subtract the average of data set
			tempData.generateRandomSubMatrix();//U and V sub-matrix of matrix factorization
			// SimpleTool.printMatrix(tempData.subU);
			// SimpleTool.printMatrix(tempData.subV);
			tempData.setRandomNoiseDistribution();//Initialize a random noise distribution
			tempData.computeWeight();// For the first time. Round 0

			MLGMDN tempEm = new MLGMDN(tempData);
			// tempEm.setModel();

			tempEm.iterationEM(tempData.maxIterTimes);//EM algorithm
...
//			SimpleTool.printMatrix(tempEm.z);
			...
			double[][][] tempTrueBasedCondPro = tempCP.computeTrueBasedCondProWithInteg(tempEm.sigma);
			...
			MBCompution tempMBCom = new MBCompution();
